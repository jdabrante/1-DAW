...
class MailServer(DbUtils):
    ...
    def send_mail(self, *, recipient: str, subject: str, body: str) -> None:
        ...
        como el objeto actual de tipo MailServer."""
        regex = r"^\w+@{1}\w+\.\w+$"
        if re.match(regex, recipient, re.I) is None:
    ...
    def get_emails(self, sent: bool = True):
        ...
        if sent:
            rows = self.cur.execute(
                f"SELECT * FROM activity WHERE sender = '{self.sender}'"
            )
        else:
            rows = self.cur.execute(
                f"SELECT * FROM activity WHERE recipient = '{self.sender}'"
            )
        for row in rows:
            sender = row["sender"]
            ...
            subject = row["subject"]
            body = row["body"]
            yield Mail(sender, recipient, subject, body)
