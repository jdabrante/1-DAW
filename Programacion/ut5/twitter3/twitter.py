...
def create_db(db_path: str = DB_PATH) -> None:
    ...
    cur.execute(create_table_tweet)
    con.commit()
    con.close()
...
class User:
    ...
    def login(self, password: str) -> None:
        ...
        row = self.cur.execute(sql,(self.username,password))
        row = row.fetchone()
        result = row[0]
        self.logged = True if result > 0 else False
    ...
    def from_db_row(cls, row: sqlite3.Row):
        ...
        user = User(username,password,bio=bio,user_id=user_id)
        return user
...
class Tweet:
    ...
    def content(self) -> str:
        ...
        if self.is_retweet:
            ...
            row = self.cur.execute(sql,(self.retweet_from,))
            row = row.fetchone()
            content = row['content']
            return content
        return self._content
    ...
    def from_db_row(cls, row: sqlite3.Row) -> Tweet:
        ...
        new_tweet = Tweet(content=content,retweet_from=retweet_from,tweet_id=tweet_id)
        return new_tweet
...
class Twitter:
    ...
    def get_user(self, user_id: int) -> User:
        ...
        result = self.cur.execute(sql,(user_id,)).fetchone()[0]
        if result == 0:
